...
void printToFile(double input_array[], char* filename, const char* name){
...
void DarkMatterGPlot(char* filename){
  // read in data
  vector<Double_t> vx;
  vector<Double_t> vy;
  Double_t xdat,ydat;
  ...
  printToFile(rXV,filename1,"_rXV.txt");
  printToFile(rYT,filename2,"_rYT.txt");
  printToFile(xxerror,filename3,"_xxerror.txt");
  printToFile(yTerror,filename4,"_yTerror.txt");
...
void batch_plot(){
  ...
  char prefix[]={"GS4_Trace"};
  char* GSName=strcat(prefix,integer_string);
  DarkMatterGPlot(GSName);}

  for(int i=328;i<361;i++){
  char integer_string[32];
  int integer = i;
  sprintf(integer_string, "%d", integer);
  char prefix[]={"GS4_Trace"};
